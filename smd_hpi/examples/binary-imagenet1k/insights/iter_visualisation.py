    ...
    def get_callback(self, symbol, data_list, label_list, context, model):
        ...
        def plot_visual_backprop(execution_params):
            for i, data, label in zip(range(0, len(data_list)), data_list, label_list):
                ...
                input_data_shapes = [(description.name, (batch_size,) + description.shape[1:]) for description in
                                     data_iter.provide_data]
                label_shapes = [(description.name, (batch_size,) + description.shape[1:]) for description in
                ...
                probs = executor.get_outputs()[0].asnumpy()
                prediction = np.argmax(probs)
                likeliness = np.max(probs)
                ...
                draw = ImageDraw.ImageDraw(dest_image)
                ...
                dest_image2 = Image.new(
                    "RGB",
                    (image.width + visualization_image.width + composite.width,
                     max(image.height, visualization_image.height))
                )
                ...
                draw = ImageDraw.ImageDraw(dest_image2)
                draw.text((0, 200), CLASSES[prediction] + " ({:.0f}%)".format(likeliness * 100), font=font)
